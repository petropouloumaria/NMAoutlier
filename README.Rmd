...
- **Several outlier and influence detection measures considered deletion:** Raw, Standardized,Studentized deleted residuals; 
  Cook distance; COVRATIO; weight “leave one out”; leverage “leave one out”; heterogeneity “leave one out”; 
  R heterogeneity; R Qtotal; R Qheterogeneity;  R Qinconsistency and DFBETAS.
...
- **Forward plots (fwdplot) for the monitoring statistics** in each step of forward search algorithm
  P-scores; z-values for difference of direct and indirect evidence with back-calculation method; 
...
You can calculate some simply outlier detection measures with function **NMAoutlier.measures** as follows:
```{r, eval=FALSE}
...
You can plot the Mahalanobis distance for each study with function **measplot** as follows:
```{r, eval=FALSE}
...
You can calculate some outlier detection measures considered deletion with function **NMAoutlier,measures** as follows:
```{r, eval=FALSE}
...
We can plot the R statistic for Qinconsistency with function **measplot** as follows:
```{r, eval=FALSE}
...
You can conduct the random shift variance model for each study with function **NMAoutlier.rsv** as follows: 
```{r, eval=FALSE}
...
You can plot the of Likelihood Ratio Test (LRT) of random shift variance model for each study with function **rsvplot** as follows:
```{r, eval=FALSE}
rsvplot(RSVresult, "LRT")
```

You can see the plots for summary estimates for each treatment B, C and D with function **rsvplotest**  as follows: 
```{r, eval=FALSE}
