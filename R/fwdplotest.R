...
#'
#' @description Forward plots of summary estimate with 95 percent
...
#' 
#' @details Plot of summary estimates and their confidence intervals
#'   for each iteration of search.  Vertical axis provides iterations
#'   of search. Horizontal axis provides summary estimate of a
#'   treatment.
#'
#' @examples
...
fwdplotest <- function(x) {
  ...
  ## (j counts from 2)
  for (j in 2:nt) {
    local({ ## localizing variables - workaround for ggplot problem
            ## with handling variables in multiple plots (variable
            ## environment scope problem)
